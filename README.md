...
3. 对每个文件都取出相应topK的K个<URL，count>根据count降序排列，形成类似一个数组，将100个文件的Top K对写入内存。
...
***
URL的长度有限，分布可能较密集的情况下，Trie可能可以存放在内存中。对每条URL统计完成后，在Trie上做DFS即可。
...
问题分析：0和1随机生成，可以理解为二进制。可以令a=foo()*2^3+foo()*2^2+foo()*2^1+foo()等概率生成0-15的所有数，只取1~10之间的数，则产生1到10之间的数仍为等概率。1~3时返回0，4~10时返回1
```Java
